...
async def website_home(request):
    response = web.FileResponse(WEBSITE_DIR / "index.html")
...
async def website_dashboard(request):
    if not current_web_admin(request):
        response = web.FileResponse(WEBSITE_DIR / "dashboard-login.html")
    else:
        response = web.FileResponse(WEBSITE_DIR / "dashboard.html")
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    return response
...
async def website_asset(request):
    ...
    response = web.FileResponse(WEBSITE_DIR / filename)
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    return response
