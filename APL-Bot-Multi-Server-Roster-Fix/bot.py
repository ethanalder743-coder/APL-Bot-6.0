...
def get_team_players(role, manager_id):
    return [member for member in role.members if not member.bot and member.id != manager_id]
...
class TeamBot(commands.Bot):
    async def setup_hook(self):
        ...
        print(f"Synced {len(global_synced)} global commands and {len(guild_synced)} test-server commands")
