...
from byronic_features import ERROR_COLOR, brand_embed, setup_byronic
...
async def send_log(guild, title, description, color=discord.Color.blurple()):
    ...
    if not channel:
        return
    embed = brand_embed(title=title, description=description, color=color, timestamp=discord.utils.utcnow())
...
class TeamBot(commands.Bot):
    async def setup_hook(self):
        ...
        if GUILD_ID:
            ...
            # Guild commands appear immediately in the main server while the
            # matching global commands propagate to every other server.
            self.tree.copy_global_to(guild=guild)
            guild_synced = await self.tree.sync(guild=guild)
        print(f"Synced {len(global_synced)} global commands and {len(guild_synced)} instant main-server commands")
...
@bot.tree.error
async def slash_command_error(interaction, error):
    original = getattr(error, "original", error)
    if isinstance(error, app_commands.CommandOnCooldown):
        message = f"Try again in **{error.retry_after:.1f} seconds**."
    elif isinstance(error, app_commands.TransformerError):
        message = "One of the selected users, roles, channels or values is invalid. Please select it from Discord's command menu."
    elif isinstance(original, discord.Forbidden):
        message = "Discord blocked that action. Move the bot role higher and check its server permissions."
    elif isinstance(original, discord.NotFound):
        message = "That Discord user, role, channel or message no longer exists."
    else:
        message = "The command could not finish. Check Railway logs for the error details."
        print(f"Slash command error in /{interaction.command.name if interaction.command else 'unknown'}: {original!r}")
    await reply(interaction, brand_embed("Command Error", message, color=ERROR_COLOR))
...
class TotwControlsView(discord.ui.View):
    ...
    async def calculate(self, interaction: discord.Interaction, button):
        ...
        embed = brand_embed(title="APL Team of the Week", description=list_text, color=discord.Color.green())
...
async def on_member_join(member):
    ...
    if channel:
        ...
        embed = brand_embed(title="Welcome!", description=message, color=discord.Color.green())
...
@bot.tree.command(name="removeteam", description="Remove a team from the bot")
...
async def removeteam(interaction: discord.Interaction, team_role: discord.Role):
    ...
    await reply(interaction, f"Removed **{team_role.name}** from the bot. The Discord role was not deleted.")
...
async def command_list(interaction: discord.Interaction):
    embed = brand_embed(
        ...
        description="Here are the commands available in this bot.",
        color=discord.Color.blurple(),
    )
    embed.add_field(
        name="⚽ Team Management",
        value=(
            "`/myoffers` · `/teaminfo` · `/teams` · `/roster`\n"
            "`/offer` · `/canceloffer` · `/teamoffers` · `/release`\n"
            "`/promote` · `/demote` · `/gametime`"
        ),
        inline=False,
    )
    embed.add_field(name="⭐ Leveling & Statistics", value="`/rank` · `/leaderboard` · `/messages` · `/messageleaderboard`\n`/invites` · `/inviteleaderboard` · `/userinfo`", inline=False)
    embed.add_field(name="🎉 Giveaways", value="`/gstart` · `/g-end` · `/g-reroll`", inline=False)
    embed.add_field(name="🔗 Help", value="`/help` · `/commands` · `/website`", inline=False)
    if is_saved_admin(interaction.user.id, interaction.guild) or is_admin(interaction):
        embed.add_field(
            name="⚙️ Setup Commands (Admin)",
            value="`/dashboard` · `/automod_setup` · `/warnthreshold` · `/ghostping`\n`/ticketsetup` · `/applicationsetup` · `/welcomesetup` · `/farewellsetup`\n`/leveling` · `/levelroles` · `/setup_starboard` · `/giveawaytoggle`",
            inline=False,
        )
        embed.add_field(
            name="🔨 Moderation Commands",
            value="`/warn` · `/kick` · `/ban` · `/removeban` · `/timeout`\n`/removetimeout` · `/cleanchat` · `/warnings` · `/clearwarnings`\n`/lockdown` · `/unlock` · `/servercase`",
            inline=False,
        )
        embed.add_field(
            name="📊 Admin Statistics & Tracking",
            value="`/setxp` · `/resetxp` · `/addinvite` · `/removeinvite`\n`/addbonus` · `/resetinvites` · `/message_invite_toggle`\n`/ignoredrole` · `/ignoredchannel`",
            inline=False,
        )
        embed.add_field(
            name="🛠️ APL Administration",
            value="`/addteam` · `/removeteam` · `/allrosters` · `/ticketpanel`\n`/addticketbutton` · `/deleteticketbutton` · `/setticketpings`\n`/totw` · `/rules` · `/managerlist` · `/setsigningschannel`",
            inline=False,
        )
    embed.set_footer(text="APL Bot • Commands shown are based on your permissions")
...
async def myoffers(interaction: discord.Interaction):
    ...
    embed = brand_embed(title="Your Pending Offers", description="\n".join(lines), color=discord.Color.blurple())
...
async def teaminfo(interaction: discord.Interaction):
    ...
    embed = brand_embed(title=role.name, description="APL Team Information", color=role.color if role.color.value else discord.Color.blurple())
...
async def roster(interaction: discord.Interaction):
    ...
    embed = brand_embed(title=f"{role.name} Roster", description=roster_text, color=role.color if role.color.value else discord.Color.blurple())
...
async def ownerlist(interaction: discord.Interaction):
    ...
    embed = brand_embed(title="APL Bot Administration", color=discord.Color.gold())
...
async def teamoffers(interaction: discord.Interaction):
    ...
    embed = brand_embed(title=f"{role.name if role else 'Team'} Pending Offers", description=text, color=discord.Color.blurple())
    await reply(interaction, embed)
...
async def allrosters(interaction: discord.Interaction):
    ...
    for team in saved_teams:
        ...
        embed = brand_embed(title=f"{role.name} Roster", color=role.color if role.color.value else discord.Color.blurple())
...
async def rules(interaction: discord.Interaction, channel: discord.TextChannel, rules: str, title: str = "Server Rules"):
    if not is_admin(interaction):
        ...
        return
    embed = brand_embed(title=title, description=rules, color=discord.Color.gold())
...
async def managerlist(interaction: discord.Interaction, channel: discord.TextChannel, message: str):
    if not is_admin(interaction):
        ...
        return
    embed = brand_embed(title="APL Manager List", description=message, color=discord.Color.blurple())
...
async def gametimeroles(interaction: discord.Interaction):
    ...
    embed = brand_embed(title="Game-Time Command Roles", description=text, color=discord.Color.blurple())
    await reply(interaction, embed)
