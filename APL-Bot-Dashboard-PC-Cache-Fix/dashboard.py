...
DASHBOARD_VERSION = "2026.08.17.2"
...
<div class="notice"><div class="bot-icon">APL</div><div><b>You're configuring APL Bot</b><span>Settings on this page apply to {html.escape(guild.name)} and save directly to the bot. Dashboard version {DASHBOARD_VERSION}.</span></div></div>
...
async def setup_dashboard(bot, connect_db, get_setting, set_setting, is_admin, member_is_admin, reply):
    ...
    async def dashboard_get(request):
        ...
        return web.Response(
            text=page(guild, token, lambda key, default="": read_value(guild,key,default), request.query.get("saved") == "1"),
            content_type="text/html",
            headers={"Cache-Control": "no-store, no-cache, must-revalidate, max-age=0", "Pragma": "no-cache", "X-Dashboard-Version": DASHBOARD_VERSION},
        )
    ...
    async def health(_):
        return web.json_response(
            {"ok": True, "bot_ready": bot.is_ready(), "guilds": len(bot.guilds), "dashboard_version": DASHBOARD_VERSION},
            headers={"Cache-Control": "no-store"},
        )

    async def retire_service_worker(_):
        script = "self.addEventListener('install',()=>self.skipWaiting());self.addEventListener('activate',event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.map(key=>caches.delete(key)))).then(()=>self.registration.unregister()).then(()=>self.clients.claim())));"
        return web.Response(text=script, content_type="application/javascript", headers={"Cache-Control": "no-store, max-age=0", "Service-Worker-Allowed": "/"})
    ...
    app.router.add_get("/service-worker.js", retire_service_worker)
