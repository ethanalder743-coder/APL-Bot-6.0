...
def is_saved_admin(user_id):
    if is_bot_owner(user_id):
        return True
    with connect_db() as db:
        return db.execute("SELECT 1 FROM bot_admins WHERE user_id = ?", (user_id,)).fetchone() is not None


def member_is_admin(member):
    return bool(member and (member.guild_permissions.administrator or is_saved_admin(member.id)))


async def send_log(guild, title, description, color=discord.Color.blurple()):
    channel_id = int(get_setting("log_channel_id", "0") or 0)
...
async def on_member_remove(member):
    if get_setting("rolesaver_enabled", "false") != "true":
        return
    with connect_db() as db:
        ...
        if saved:
            ...
            if roles:
                try:
                    ...
                    pass
    channel_id = int(get_setting("welcome_channel_id", "0") or 0)
    channel = member.guild.get_channel(channel_id)
    if channel:
        template = get_setting("welcome_message", "Welcome {user} to **{server}**!")
...
async def addteam(interaction: discord.Interaction, manager: discord.Member, team_role: discord.Role):
    if not is_saved_admin(interaction.user.id):
...
async def command_list(interaction: discord.Interaction):
    ...
    if is_saved_admin(interaction.user.id) or is_admin(interaction):
        embed.add_field(
...
async def addadmin(interaction: discord.Interaction, member: discord.Member):
    if not is_bot_owner(interaction.user.id):
        await reply(interaction, "Only a configured bot owner can add bot administrators.")
        return
    with connect_db() as db:
        db.execute("INSERT OR IGNORE INTO bot_admins (user_id) VALUES (?)", (member.id,))
...
async def removeadmin(interaction: discord.Interaction, member: discord.Member):
    if not is_bot_owner(interaction.user.id):
        await reply(interaction, "Only a configured bot owner can remove bot administrators.")
        return
    with connect_db() as db:
        db.execute("DELETE FROM bot_admins WHERE user_id = ?", (member.id,))
...
async def setlogchannel(interaction: discord.Interaction, channel: discord.TextChannel):
    if not is_admin(interaction):
        ...
        return
    set_setting("log_channel_id", channel.id)
...
async def allrosters(interaction: discord.Interaction):
    if not is_admin(interaction):
        ...
        return
    with connect_db() as db:
        saved_teams = db.execute("SELECT * FROM teams").fetchall()
    embeds = []
    for team in saved_teams:
        role = interaction.guild.get_role(team["team_role_id"])
        if not role:
            continue
        players = [member for member in role.members if not member.bot and member.id != team["manager_id"]]
...
async def setwelcome(interaction: discord.Interaction, channel: discord.TextChannel, message: str):
    if not is_admin(interaction):
        ...
        return
    set_setting("welcome_channel_id", channel.id)
    set_setting("welcome_message", message)
...
async def rolesaver(interaction: discord.Interaction, enabled: bool):
    if not is_admin(interaction):
        ...
        return
    set_setting("rolesaver_enabled", "true" if enabled else "false")
...
async def setgametimechannel(interaction: discord.Interaction, channel: discord.TextChannel):
    if not is_admin(interaction):
        ...
        return
    set_setting("gametime_channel_id", channel.id)
...
async def gametime(interaction: discord.Interaction, opponent: str, date: str, time: str, note: str = ""):
    ...
    if not is_admin(interaction) and not allowed_role_ids.intersection(member_role_ids):
        ...
        return
    channel_id = int(get_setting("gametime_channel_id", "0") or 0)
