...
async def save_website_data(request):
    if not WEB_ADMIN_KEY:
        raise web.HTTPServiceUnavailable(text="WEB_ADMIN_KEY is not configured")
    supplied_key = request.headers.get("X-Admin-Key", "")
    if not hmac.compare_digest(supplied_key, WEB_ADMIN_KEY):
...
async def check_website_admin(request):
    supplied_key = request.headers.get("X-Admin-Key", "")
    if not WEB_ADMIN_KEY or not hmac.compare_digest(supplied_key, WEB_ADMIN_KEY):
        raise web.HTTPUnauthorized(text="Invalid admin key")
...
async def start_website(client):
    ...
    app.router.add_get("/api/admin-check", check_website_admin)
