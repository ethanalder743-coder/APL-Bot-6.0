...
The website includes a responsive 2026 APL visual system with floating navigation, upgraded hero and cards, readable standings, live team cards, TOTW, cup navigation/brackets, mobile layouts, and branded social-sharing artwork.

Website access uses email invitations instead of a shared password. `WEB_OWNER_EMAIL` is the permanent website owner. Use `/websiteaccess` privately in Discord for the owner's first sign-in. Every signed-in website administrator can open **Email Admin Access**, enter another administrator's email and press **Create access email**. Their normal email app opens with the secure link and message ready to send, and the link can also be copied. The invited email remains an approved administrator after clicking the link.

The website Admin panel also controls the public player-statistics dashboard. Add each player's club, goals, assists and MOTM awards to update the Stats tab automatically.

The bot control dashboard is a separate private website at `/dashboard`. Use `/websiteaccess` to sign in. It configures server channels, manager/team setup, ticket buttons and questions, moderation guidance, welcome messages, role saver, Game Time permissions, logs, TOTW uploads and administrator invitations. The public league website remains at `/`.

## What it does

- `/addteam manager team_role logo` links that manager to exactly one team role, gives them both roles, and automatically adds the club name, manager and optional uploaded badge to the website. Running it again updates the saved team or logo.
