...
Website access uses email invitations instead of a shared password. `WEB_OWNER_EMAIL` is the permanent website owner. The owner can enter another email in the admin console; that person receives a one-time link and remains an approved administrator after clicking it. Approved admins can request another link if they change devices or clear their browser data.
...
Copy that complete generated address into `WEB_BASE_URL`, put your own email in `WEB_OWNER_EMAIL`, and redeploy. For Gmail sending, enable two-step verification and create a Google **App Password** for `SMTP_PASSWORD`; do not use your normal Gmail password. `SMTP_USER` and `SMTP_FROM_EMAIL` should normally be the Gmail address sending the invitations.
