        ...
        if guild:
            with connect_db() as db:
                saved_teams = db.execute(
                    "SELECT team_role_id FROM teams WHERE guild_id = ?", (guild.id,)
                ).fetchall()
            for saved_team in saved_teams:
                role = guild.get_role(saved_team["team_role_id"])
                if role:
                    bot_teams.append({"id": str(role.id), "name": role.name})
...
def send_login_email_sync(email, login_url, invited=False):
    if not SMTP_USER or not SMTP_PASSWORD or not SMTP_FROM_EMAIL:
        raise RuntimeError("Email sending is not configured")
    message = EmailMessage()
    message["From"] = SMTP_FROM_EMAIL
    message["To"] = email
    message["Subject"] = "Your APL League website admin access"
    intro = "You have been invited to manage the APL League website." if invited else "Use this secure link to sign in to the APL League website."
    message.set_content(
...
async def create_and_email_login(email, invited=False):
    if not WEB_BASE_URL:
    ...
    login_url = f"{WEB_BASE_URL}/auth/verify?token={token}"
    await asyncio.to_thread(send_login_email_sync, email, login_url, invited)


async def request_admin_email(request):
    try:
    ...
    if not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", email):
        raise web.HTTPBadRequest(text="Enter a valid email address")
    with connect_db() as db:
        allowed = db.execute("SELECT 1 FROM web_admins WHERE email = ?", (email,)).fetchone()
    if allowed:
        try:
            await create_and_email_login(email)
        except Exception as error:
            print(f"Could not send website sign-in email: {error}")
            raise web.HTTPServiceUnavailable(text="Email sending is not configured correctly")
    # Always use the same public reply so visitors cannot discover admin emails.
    return web.json_response({"message": "If that email has admin access, a sign-in link has been sent."})
...
async def verify_admin_email(request):
    ...
    response = web.HTTPFound("/?admin=1")
    response.set_cookie(
        "apl_admin_session",
        session_token,
...
async def invite_website_admin(request):
    ...
    if not signed_in_email or signed_in_email != WEB_OWNER_EMAIL:
        raise web.HTTPForbidden(text="Only the website owner can invite administrators")
    try:
        payload = await request.json()
        email = str(payload.get("email", "")).strip().lower()
    except (json.JSONDecodeError, web.HTTPBadRequest, AttributeError):
    ...
    try:
        await create_and_email_login(email, invited=True)
    except Exception as error:
        print(f"Could not send website invitation: {error}")
        raise web.HTTPServiceUnavailable(text="The invitation was saved, but the email could not be sent")
    return web.json_response({"invited": True, "email": email})
...
async def start_website(client):
    ...
    app.router.add_post("/api/auth/request", request_admin_email)
